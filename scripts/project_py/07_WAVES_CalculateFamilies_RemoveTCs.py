...
# input files
p_wvs_parts = ST.WAVES.partitions_p1                # waves partitions data (GOW)
p_hist_r2_params = ST.TCs.hist_r2_params            # hist storms parameters inside r2

# output files
p_wvs_parts_noTCs = ST.WAVES.partitions_notcs       # waves partitions data without TCs (GOW)
p_wvs_fams_noTCs = ST.WAVES.families_notcs          # waves families data without TCs
p_wvs_fams_TCs_categ = ST.WAVES.families_tcs_categ  # waves families data at TCs times
...
# store waves famileis data (TCs removal window)
if not op.isdir(p_wvs_fams_TCs_categ): os.makedirs(p_wvs_fams_TCs_categ)
for k in d_wvs_fam_cats.keys():
